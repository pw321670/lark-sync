...
- 当前同步状态已经会持久化到插件数据中，插件重载后仍可继续跳过未改动文件。
- Markdown 文档模式也已经会持久化远端 `docId`，后续修改会优先原位更新同一个飞书文档。
- 当前文档更新仍是“清空根块后重建内容块”的粗粒度更新，不是更细的块级 patch。
- 如果某个远端目录里已经历史遗留了多个同标题重复文档，而本地状态又丢失，当前恢复逻辑只会复用第一个匹配项来阻止继续增殖，不会自动帮你清理旧重复件。
